...
public class Computer implements OperationCode {
...
//          codeInjector(); // sample code, sama dengan cpu.hardwired()
  ...
  public byte runInstruction(int startAddress) {
    ...
    if (powerUp) {
      // set Base address
//          cntrl_unit.setBaseAddress(startAddress);

      endExecution = cntrl_unit.executeInstruction(); // return: opcode
//          cntrl_unit.memoryContent();
      //         cntrl_unit.registerContent(); // isi register
      updateRegisters(); // untuk diakses level di atasnya
    } // true komputer ON
...
//       mainMemory.writeWordMemory(lokasi,aNumerik);
  ...
  // file ini per baris berisi nilai desimal, sehingga tidak menunjukkan ukuran file
  //   yang sesungguhnya di main memori, seharusnya dalam bentuk hexadecimal binary
  ...
  public void codeInjector(String fMachineCode, int startLocation) throws IOException {
    ...
    while ((s = in.readLine()) != null) {
      tData = Integer.parseInt(s);
      //            System.out.printf("Computer codeinjector lokasi %08x tData -> %08x (%d)\n",lokasi,tData,tData);
      cpu.writeWordMemory(lokasi, tData); // akses melalui cpu
      lokasi = lokasi + 4;
    } // endwhile readLine
