...
public class Processor_modif implements OperationCode {
  // Registers
  ...
  public int BP; // base pointer register untuk menunjukkan lokasi dasar (awal) stack memori
...
/*off
       ...
       int lokasi;
       lokasi=0;
       RAM.writeByteMemory(lokasi,(byte)45);
       lokasi=4;
       RAM.writeByteMemory(lokasi,(byte)100);
       int tData;
       byte[] aNumerik;
       aNumerik = new byte[4];
       tData = 0x275B8A5F;
...
//
  ...
  Processor_modif(Endian pEndian) {
//      System.out.println("Register -> "+Register.r8.toString());
    defStartLoc = 0; // start address to feth first instruction
    PC = defStartLoc;
...
//    littleEndian sebagai basis eksekusi
  ...
  public void in(byte[] p) {
    ...
    if (regSrc1 == 0) {
      aRegister[regDest] = (int) inpByte;
      for (int k = 0; k < 100000; k++) for (int m = 0; m < 30000; m++) ;
    }
  }
...
//       System.out.printf("Operasi MOVML a[regDest] : %x\n",aRegister[regDest]);
  ...
  // keluarkan data dari stack ke register
  public void pop() {
